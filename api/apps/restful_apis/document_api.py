...
from api.apps.restful_apis.document_delete_helpers import find_invalid_doc_ids
...
async def delete_documents(tenant_id, dataset_id):
    ...
    try:
        ...
        if delete_all:
            # ids 由本库自身产出，天然合法，无需再校验归属（原实现会为此再全表扫一遍）。
            doc_ids = [doc.id for doc in DocumentService.query(kb_id=dataset_id)]
        else:
            # HugAI 性能补丁：原实现把整库文档全量 ORM 实例化只为拿 id 集合做归属校验，
            # 复杂度 O(库内文档数)、与本次删几个无关（76 万文档实测 ~49s，且同步查询卡死
            # 单进程 API）。改为把请求的 id 下推做定向查询 `WHERE kb_id=? AND id IN (...)`，
            # 由 id 主键定位（请求列表极具选择性）、kb_id 回表过滤，O(请求数)、毫秒级；
            # invalid_ids 语义与报错文案不变。
            def _query_existing(kb_id, ids):
                return [d.id for d in Document.select(Document.id)
                        .where((Document.kb_id == kb_id) & (Document.id.in_(ids)))]

            invalid_ids = find_invalid_doc_ids(dataset_id, doc_ids, _query_existing)
            if invalid_ids:
                return get_error_data_result(
