...
class DocumentService(CommonService):
    ...
    @classmethod
    @DB.connection_context()
    def get_by_kb_id(cls, kb_id, page_number, items_per_page, orderby, desc, keywords, run_status, types, suffix=None, name=None, doc_ids=None, return_empty_metadata=False, need_count=True):
        # HugAI 性能补丁：过滤条件全部只落在 Document 表，四张 JOIN 表(File2Document/File/
        # UserCanvas/User)只提供展示列(pipeline_name/nickname)。故把「WHERE 过滤」抽成一个
        # 可复用闭包，分别作用于两条查询：
        #   · 列表查询：带 JOIN 取展示列，但只对当页(分页后)物化 —— 展示 JOIN 只作用于 30 行；
        #   · 计数查询：纯 Document 表、不带任何 JOIN —— 原实现 `docs.count()` 会把带 4 JOIN 的
        #     select 整包成 `SELECT COUNT(1) FROM (...)`，76 万文档库上 3~4 秒且同步卡死单进程 API。
        # 去掉 JOIN 后 count 对正常数据不变(画像/上传文档都成对建了 file2document+file)，仅对
        # 残链脏数据会「变大」= 修正原本偏小的统计。need_count=False 供不使用 total 的调用点跳过计数。
        fields = cls.get_cls_model_fields()

        def _apply_filters(query, doc_ids_with_metadata):
            if keywords:
        ...
        # return_empty_metadata 需要「本库有 metadata 的文档 id 集合」，列表与计数共用同一份。
        ...
        docs = _apply_filters(docs, doc_ids_with_metadata)

        count = 0
        if need_count:
            # 纯 Document 表、同一组 WHERE、不带 JOIN 的独立计数查询
            count_query = _apply_filters(cls.model.select(cls.model.id).where(cls.model.kb_id == kb_id), doc_ids_with_metadata)
            count = count_query.count()

        order_field = cls.model.getter_by(orderby)
        order_expr = order_field.desc() if desc else order_field.asc()

        if page_number and items_per_page:
            # 延迟关联：4 表 JOIN 对 76 万行整表分页(JOIN 完再排序取 30 行)实测 12s。
            # 改为先在纯 Document 表上过滤+排序+分页拿当页 id(kb_id/create_time 均有索引，
            # 0.19s)，再对当页 id 做展示 JOIN(30 行，0.005s)。展示查询 docs 此前已被
            # _apply_filters 施加过全部过滤，这里在其上叠加 id 白名单——当页 id ⊆ 过滤后
            # 集合，叠加幂等无害。
            page_ids_query = _apply_filters(cls.model.select(cls.model.id).where(cls.model.kb_id == kb_id), doc_ids_with_metadata)
            page_ids = [d.id for d in page_ids_query.order_by(order_expr).paginate(page_number, items_per_page)]
            if not page_ids:
                # 页码越界等空页场景直接短路，避免空 IN 的方言差异
                docs_list = []
                return docs_list, count
            docs = docs.where(cls.model.id.in_(page_ids)).order_by(order_expr)
        else:
            # 全量拉取(如 run_index/run_embedding)保持原查询形态：延迟关联对全量无意义
            docs = docs.order_by(order_expr)
