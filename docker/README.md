...
- `REDIS_KEY_PREFIX`
  Required in cluster mode. Isolates all RAGFlow keys when the Redis Cluster is shared with other applications. Do not include `{` or `}`.
- `REDIS_PORT`  
...
Example for a shared Redis Cluster:

```dotenv
REDIS_MODE=cluster
REDIS_DB=0
REDIS_CLUSTER_NODES=redis-1:7001,redis-2:7001,redis-3:7001
REDIS_KEY_PREFIX=hugai:knowledge-engine:prod:
REDIS_USERNAME=
REDIS_PASSWORD=<load-from-your-secret-management-system>
```

The password above is intentionally not stored in the repository. Keep `REDIS_KEY_PREFIX` unique per environment; changing it makes the application use a different logical key namespace.
