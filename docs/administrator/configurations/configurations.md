...
- `REDIS_MODE`
  Redis connection mode. Use `standalone` (default) or `cluster`.
- `REDIS_DB`
  Logical database used in standalone mode. Redis Cluster always uses database `0`.
- `REDIS_CLUSTER_NODES`
  Comma-separated `host:port` startup nodes used in cluster mode. The client discovers the remaining topology automatically.
- `REDIS_KEY_PREFIX`
  Required in cluster mode. Isolates RAGFlow keys when the Redis Cluster is shared with other applications. Do not include `{` or `}`.
- `REDIS_PORT`
...
- `mode`: Redis connection mode. Options are `standalone` (default) and `cluster`.
- `host`: The Redis serving IP *and* port inside the Docker container. Defaults to `redis:6379`.
- `db`: The Redis database index used in standalone mode. Defaults to `1`. Redis Cluster always uses database `0`.
- `nodes`: Comma-separated `host:port` startup nodes used in cluster mode.
- `key_prefix`: Required in cluster mode to isolate keys on a shared cluster. Do not include `{` or `}`.
