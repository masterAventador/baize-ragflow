...
def _checkpoint_slot_tag(tenant_id: str, kb_id: str, checkpoint_type: str) -> str:
    """让同一批 checkpoint 数据与索引在 Redis Cluster 中落到同一 Slot。"""
    return f"{{{tenant_id}:{kb_id}:{checkpoint_type}}}"


def _checkpoint_index_key(tenant_id: str, kb_id: str, checkpoint_type: str) -> str:
    return f"graphrag:checkpoint:{_checkpoint_slot_tag(tenant_id, kb_id, checkpoint_type)}:keys"


def _checkpoint_data_key(tenant_id: str, kb_id: str, checkpoint_type: str, checkpoint_key: str) -> str:
    return f"graphrag:checkpoint:{_checkpoint_slot_tag(tenant_id, kb_id, checkpoint_type)}:{checkpoint_key}"
...
def _iter_checkpoint_keys(index_key: str, page_size: int | None):
    return REDIS_CONN.sscan_iter(index_key, count=_checkpoint_page_size(page_size))
...
async def save_checkpoint(tenant_id: str, kb_id: str, checkpoint_type: str, checkpoint_key: str, payload: Any) -> bool:
    ...
    try:
        REDIS_CONN.save_indexed_value(
            data_key=data_key,
            value=json.dumps(payload, ensure_ascii=False),
            index_key=index_key,
            member=checkpoint_key,
