...
def _bulk_refresh():
    """Return ES bulk refresh mode.

    Upstream RAGFlow defaults to ``wait_for``. HugAI's short-lived initial import branch
    can set ``HUGAI_BULK_IMPORT_ES_REFRESH=false`` to avoid waiting for every ES refresh.
    """
    value = os.getenv("HUGAI_BULK_IMPORT_ES_REFRESH", "wait_for").strip().lower()
    if value in {"false", "0", "no", "off"}:
        return False
    if value in {"true", "1", "yes", "on"}:
        return True
    return value or "wait_for"


def _delete_refresh():
    """Return refresh flag for ES delete-by-query.

    ``delete_by_query`` only accepts boolean refresh semantics. Treat the upstream
    bulk default ``wait_for`` as ``True`` and let HugAI bulk import disable it with
    the same ``HUGAI_BULK_IMPORT_ES_REFRESH=false`` switch.
    """
    return bool(_bulk_refresh())
        ...
        for _ in range(ATTEMPT_TIME):
            try:
                ...
                r = self.es.bulk(index=index_name, operations=operations,
                                 refresh=_bulk_refresh(), timeout="60s")
